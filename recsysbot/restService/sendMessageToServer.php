...
function sendMessageToServer($chatId, $messageId, $timeStamp, $text, $firstname, $botName) {
	...
	$stringGetRequest = $config['application_uri'] . '/restService/chatMessage/sendMessageToServer/send?userID=' . $userID . '&messageID=' . $messageId . '&timeStamp=' . $timeStamp . '&text=' . urlencode($text) . '&firstname=' . $firstname . '&botName=' . $botName;

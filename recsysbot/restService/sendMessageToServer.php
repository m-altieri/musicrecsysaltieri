...
function sendMessageToServer($chatId, $messageId, $timeStamp, $text, $firstname, $botName) {
	...
	$stringGetRequest = $config['application_uri'] . '/restService/sendMessageToServer?userID=' . $userID . '&messageID=' . $messageId . '&timeStamp=' . $timeStamp . '&text=' . urlencode($text) . '&firstname=' . $firstname . '&botName=' . $botName;
